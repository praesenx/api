<template>
  <article class="py-5 border-b border-slate-100 dark:border-slate-800">
    <div class="flex items-start">
      ...
      <div>
        <div class="text-xs text-slate-500 uppercase mb-1"><span class="text-sky-500">—</span> {{item.date}}</div>
        <h3 class="font-aspekta text-lg font-[650] mb-1">
          <router-link class="inline-flex relative hover:text-sky-500 duration-150 ease-out before:scale-x-0 before:origin-center before:absolute before:inset-0 before:bg-sky-200 dark:before:bg-sky-500 before:opacity-30 before:-z-10 before:translate-y-1/4 before:-rotate-2 hover:before:scale-100 before:duration-150 before:ease-in-out" :to="item.slug">{{item.title}}</router-link>
        </h3>
        <div class="flex">
          <div class="grow text-sm text-slate-500 dark:text-slate-400">
            {{item.excerpt}}
          </div>
          <router-link class="hidden lg:flex shrink-0 text-sky-500 items-center justify-center w-12 group" :to="item.slug" tabindex="-1">
